            ...
            if guild.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        '나는 광장채널에 없어...  '
                        ' %sj 를 쳐서 광장 채널로 보내줘!' % self.config.command_prefix)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.members and author_perms.skip_when_absent:
                ...
                newmsg = '지금 나오는 노래는 `%s`: `%s` 신청한 사람은 `%s`' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
        else:
...
#######################################################################################################################
    ...
    async def cmd_p(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        플레이리스트에 곡을 추가해 줄꼐. 링크도 가능해 없다면, 유튜브 검색결과를 가져와도 되지?
    ...
    async def cmd_j(self, channel, guild, author, voice_channel):
        """
        Usage:
            {command_prefix}j
        ...
        if not author.voice:
            raise exceptions.CommandError(self.str.get('cmd-summon-novc', '광장 채널에서 불러주라구!'))
